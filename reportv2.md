...
This threat model focuses on three foundational components of a city-building game platform: **API Gateway**, **Auth Service**, and **External Payment Service**. These components were chosen because they provide root-level security as if they are compromised, attackers can impersonate players, exhaust the platform, or abuse in-game purchases. The analysis uses STRIDE and addresses the priorities Financial Loss, Denial of Service (DoS), and Safe Environment.

**Key findings**: 10 design-level threats were raised. The threats span 4 Critical, 6 High across API Gateway, Auth Service, and External Payment Service. The most urgent risks are: payment–identity binding (ensuring every payment is tied to a verified user from Auth); account takeover and session theft via Auth (credential stuffing, stolen tokens); platform-wide DoS if the Gateway or Auth is exhausted; and PII/minors data exposure in Auth (COPPA/GDPR-K).

# Diagram

The diagram (Annex A) shows the city-building game architecture: trust zones (Internet, AWS, Private Subnet, Trusted Partner), main components (Browser, WAF, API Gateway, Auth Service, 3rd Party Payment Service, and other game services), and data flows. The **in-scope components** (API Gateway, Auth Service, External Payment Service) and their roles are defined in Scope. Key flows for this threat model: Browser → WAF → API Gateway → Auth (credentials, tokens) and API Gateway → 3rd Party Payment Service (payment requests, tokenized references only). Threats and mitigations assume this architecture.
...
* **Auth Service**: The component that verifies player identity, manages logins and sessions, and stores age flags for child protection. We analyze risks of impersonation, account takeover , and exposure of minor players' data.    
* **External Payment Service**: The third-party provider that processes in-game purchases. We analyze risks of unauthorized charges, refund abuse, and payment flows that are not correctly tied to the authenticated player.
...
Scope was defined around the three components above because they provide foundational level security for the game platform. Additionally, these assets are of interest because the participant is focused on threat modeling foundational services to learn more about foundation security controls and threats to these services. Within that scope, the priorities are mapped as:

* **Financial Loss** is in scope via Auth (account takeover, identity binding) and Payment (unauthorized purchases, secure processing, refunds).  
* **Denial of Service** is in scope via API Gateway (rate-limiting, resilience, platform-wide availability) and Auth (login/register exhaustion).  
* **Safe Environment** is partially in scope via Auth (age flagging, session security for minors, and identity/MFA as a control against misuse).

# Assumptions

* Infrastructure is hosted on AWS  
* WAF is deployed in front of all requests to the API gateway  
* All in-scope assets (Auth Service, API Gateway, External Payment Service) are assumed to be hardened and follow security best practices (e.g. patching, secure config, encryption at rest).  
...
The system description notes some elements may be incomplete. The assumptions above resolve:

* **Infrastructure and WAF** – AWS and WAF in front of the Gateway (assumptions 1–2) keep recommendations concrete and DDoS mitigation in scope.  
* **Payment flow and CHD** – Where card data enters was unspecified; we assumed hosted page or hosted fields so CHD never touches the platform (assumption 6).  
* **Token and identity model** – Issuance and validation were unspecified; we assumed Auth signs tokens (e.g. JWT RS256/ES256) and services verify signature and claims (assumption 5).
...
* SM3 (Session Management) – Stolen session used for maximum duration. [https://cornucopia.owasp.org/cards/SM3](https://cornucopia.owasp.org/cards/SM3)  
* SM9 (Session Management) – Session identifiers/tokens stolen via insecure channels or logging. [https://cornucopia.owasp.org/cards/SM9](https://cornucopia.owasp.org/cards/SM9)  
* SMJ (Session Management) – Reuse of stolen session identifiers without proof of possession. [https://cornucopia.owasp.org/cards/SMJ](https://cornucopia.owasp.org/cards/SMJ)
...
* AT8 (Authentication) – Authentication bypass; fails to deny by default. [https://cornucopia.owasp.org/cards/AT8](https://cornucopia.owasp.org/cards/AT8)  
* ATJ (Authentication) – No authentication requirement or missing auth. [https://cornucopia.owasp.org/cards/ATJ](https://cornucopia.owasp.org/cards/ATJ)  
* ATX (Authentication) – Bypass due to non-standard or misconfigured auth module. [https://cornucopia.owasp.org/cards/ATX](https://cornucopia.owasp.org/cards/ATX)
...
* VE2 (Data Validation & Encoding) – Information disclosure via error messages or configuration. [https://cornucopia.owasp.org/cards/VE2](https://cornucopia.owasp.org/cards/VE2)  
* AZ3 (Authorization) – Access to sensitive info via logger, cache, or reporting. [https://cornucopia.owasp.org/cards/AZ3](https://cornucopia.owasp.org/cards/AZ3)
...
* C9 (Cornucopia) – Misuse by using a feature too fast or too frequently; resource consumption. [https://cornucopia.owasp.org/cards/C9](https://cornucopia.owasp.org/cards/C9)  
* CK (Cornucopia) – Denial of service to users. [https://cornucopia.owasp.org/cards/CK](https://cornucopia.owasp.org/cards/CK)
...
* AT2 (Authentication) – Auth functions without user awareness (e.g. stolen credentials). [https://cornucopia.owasp.org/cards/AT2](https://cornucopia.owasp.org/cards/AT2)  
* AT3 (Authentication) – Obtaining password/secrets by observation, cache, transit, or leak. [https://cornucopia.owasp.org/cards/AT3](https://cornucopia.owasp.org/cards/AT3)  
* AT7 (Authentication) – Brute force/dictionary attacks without limit. [https://cornucopia.owasp.org/cards/AT7](https://cornucopia.owasp.org/cards/AT7)  
* SM3 (Session Management) – Stolen session used for maximum duration. [https://cornucopia.owasp.org/cards/SM3](https://cornucopia.owasp.org/cards/SM3)  
* SM9 (Session Management) – Stealing tokens via insecure channels or logging. [https://cornucopia.owasp.org/cards/SM9](https://cornucopia.owasp.org/cards/SM9)
...
* VE4 (Data Validation & Encoding) – Malicious field names or data not checked in user/process context. [https://cornucopia.owasp.org/cards/VE4](https://cornucopia.owasp.org/cards/VE4)  
* AZ8 (Authorization) – Bypass business rules by altering process or control data. [https://cornucopia.owasp.org/cards/AZ8](https://cornucopia.owasp.org/cards/AZ8)

**Verification:** Pentesting: attempt to tamper the age flag. SAST/DAST: validate server-side enforcement. Compliance review: COPPA/GDPR-K controls.
...
* AT7 (Authentication) – Brute force/dictionary attacks without limit. [https://cornucopia.owasp.org/cards/AT7](https://cornucopia.owasp.org/cards/AT7)  
* C9 (Cornucopia) – Misuse by over-utilizing a feature; resource consumption. [https://cornucopia.owasp.org/cards/C9](https://cornucopia.owasp.org/cards/C9)
...
## 3\. External Payment Service Threats
...
1. Gateway/Auth verifies user Identify to payment flow (Medium).  
2. Payment/orchestrator reject requests not matching authenticated users; never trust client-supplied identity (Medium).

**OWASP Cornucopia cards:**

* AZ5 (Authorization) – Access to resources due to missing auth or excessive privileges. [https://cornucopia.owasp.org/cards/AZ5](https://cornucopia.owasp.org/cards/AZ5)  
* AZ6 (Authorization) – Access to data without permission despite access to form/page/URL. [https://cornucopia.owasp.org/cards/AZ6](https://cornucopia.owasp.org/cards/AZ6)  
* SMJ (Session Management) – Reuse of stolen tokens; no proof of possession. [https://cornucopia.owasp.org/cards/SMJ](https://cornucopia.owasp.org/cards/SMJ)
...
* C9 (Cornucopia) – Resource consumption; feature over-utilization. [https://cornucopia.owasp.org/cards/C9](https://cornucopia.owasp.org/cards/C9)  
* CK (Cornucopia) – Denial of service to users. [https://cornucopia.owasp.org/cards/CK](https://cornucopia.owasp.org/cards/CK)
...
**Cost to fix:** Medium. Server-side initiation; Role-based Access Control (RBAC); audit trail for refunds.

**Suggested mitigations:**

1. All payment and refund calls initiated server-side; tied to authenticated operator or user (Medium).  
2. Enforce "refund only for the same user or support with audit"; RBAC for admin refunds (Medium).  
3. No client-side direct Payment API keys (Low).

**OWASP Cornucopia cards:**

* AZ5 (Authorization) – Access to resources due to missing auth. [https://cornucopia.owasp.org/cards/AZ5](https://cornucopia.owasp.org/cards/AZ5)  
* AZ7 (Authorization) – Access to functions/objects without permission. [https://cornucopia.owasp.org/cards/AZ7](https://cornucopia.owasp.org/cards/AZ7)  
* AZ8 (Authorization) – Bypass business rules by altering process flow. [https://cornucopia.owasp.org/cards/AZ8](https://cornucopia.owasp.org/cards/AZ8)

**Verification:** Pentesting: attempt unauthorized refund. DAST: test refund endpoints for authorization. Design review: refund flow and RBAC.
...
This threat modeling exercise demonstrates the value of identifying design-level risks early, before they become costly to fix in production. This report and exercise should assist on:
...
**Use as input:** Use this report to define pentest scope, SAST/DAST rules, and config review checklists for the API Gateway, Auth Service, and Payment integrations.
